...
import { getListingDetail, type ListingDetail } from "~/adapters/listings"
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4 border-t pt-4">
                ...
                <div className="flex gap-3">
                    ...
                    <Button className="flex-1" onClick={onOffer}>
...
export default function ListingPage({ params }: Route.ComponentProps) {
    ...
    useEffect(() => {
        async function load() {
            ...
            if (detail?.user?.h3_index && user) {
                ...
                if (me?.h3_index) {
                    setDistanceKm(
                        gridDistance(me.h3_index, detail.user.h3_index) * 1.22,
                    )
                }
            }
            setLoading(false)
    ...
    const handleOffer = () =>
        openOffer(listing!.user_id, listing!.user?.name ?? null, {
            theirWorkIds: listing!.books?.work_id
                ? [listing!.books.work_id]
                : [],
        })
